...
def nudft1(sig, fourier_pts):
    ...
    :param fourier_pts: An array of size K represents the frequencies in Fourier space at which the Fourier transform
    is to be calculated.
    :return: An Array of size K represents the Fourier transform of sig calculated at the specified frequencies.
    """
...
def anudft1(sig_f, fourier_pts, sz):
    ...
    :param sig_f: A Fourier transform calculated at the K frequencies specified by fourier_pts
    :param fourier_pts: fourier_pts: An array of size K represents the frequencies in Fourier space at which the Fourier
    transform is to be calculated.
    :param sz: signal size
    :return: An array of size (sz) represents the adjoint Fourier transform of sig_f at frequencies fourier_pts.
    """
    grid = np.arange(np.ceil(-len(sig_f) / 2.), np.ceil(len(sig_f) / 2.)).astype(np.complex64)
...
def nudft2(im, fourier_pts):
    ...
    grid_x, grid_y = np.meshgrid(grid, grid)  # grid_y and grid_x are like matlab convensions

    pts = np.array([grid_x.flatten(), grid_y.flatten()]).astype(np.complex64)

    for i in range(fourier_pts.shape[0]):
        im_f[i] = np.dot(np.e ** (0 - 1j * np.dot(fourier_pts[i, :], pts)), im.flatten())
...
def anudft2(im_f, fourier_pts, sz):
    ...
    :param im_f: A Fourier transform calculated at the K frequencies specified by fourier_pts
    :param fourier_pts: fourier_pts: An array of size K represents the frequencies in Fourier space at which the Fourier
    transform is to be calculated.
    :param sz: image size
    :return: An array of size (sz X sz) represents the adjoint Fourier transform of im_f at frequencies fourier_pts.
    """
    ...
    for i in range(pts.shape[1]):
        im[i] = np.dot(np.e ** (0 + 1j * np.dot(fourier_pts, pts[:, i])), im_f.flatten())
...
def nudft3(vol, fourier_pts):
    ...
    grid = np.arange(np.ceil(-N / 2.), np.ceil(N / 2.)).astype(np.complex64)
    grid_x, grid_y, grid_z = np.meshgrid(grid, grid, grid)  # grid_y and grid_x are like matlab convensions

    pts = np.array([grid_x.flatten(), grid_y.flatten(), grid_z.flatten()]).astype(np.complex64)

    for i in range(fourier_pts.shape[0]):
        vol_f[i] = np.dot(np.e ** (0 - 1j * np.dot(fourier_pts[i, :], pts)), vol.flatten())
...
def anudft3(vol_f, fourier_pts, sz):
    ...
    :param vol_f: A Fourier transform calculated at the K frequencies specified by fourier_pts
    :param fourier_pts: fourier_pts: An array of size K represents the frequencies in Fourier space at which the Fourier
    transform is to be calculated.
    :param sz: image size
    :return: An array of size (sz X sz X sz) represents the adjoint Fourier transform of im_vol at frequencies fourier_pts.
    """
    ...
    for i in range(pts.shape[1]):
        vol[i] = np.dot(np.e ** (0 + 1j * np.dot(fourier_pts, pts[:, i])), vol_f.flatten())
